...
function MatchScoreCell({ match }: { match: MatchDto }) {
  ...
  return (
    <Group gap={4} justify="center" wrap="nowrap">
      <Text size="sm" fw={700} ff="monospace">{homeGoals} – {awayGoals}</Text>
...
function MatchList({ matches }: { matches: MatchDto[] }) {
  ...
  return (
    <>
      ...
      <Table.ScrollContainer minWidth={400} visibleFrom="sm">
        <Table highlightOnHover>
          <Table.Tbody>
            {sorted.map(m => (
              <Table.Tr key={m.id}>
                <Table.Td ta="right" w={180}>
                  <TeamBadge code={m.homeTeam.code} name={m.homeTeam.name} size="sm" align="right" />
                </Table.Td>
                <Table.Td w={140} ta="center"><MatchScoreCell match={m} /></Table.Td>
                <Table.Td w={180}>
                  <TeamBadge code={m.awayTeam.code} name={m.awayTeam.name} size="sm" />
                </Table.Td>
                <Table.Td ta="right" w={180}>
                  <Text size="xs" c="dimmed">
                    {new Intl.DateTimeFormat(undefined, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(m.scheduledAt))}
                  </Text>
                </Table.Td>
              </Table.Tr>
            ))}
...
function StagesPage() {
  ...
  return (
    <Container size="md">
      <Stack gap="xl">
        <PageHero emoji="⚽" title="Stages & Matches" subtitle="48 matches · 16 host cities" />
